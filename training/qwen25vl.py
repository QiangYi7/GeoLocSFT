...
def setup_argparse():
    parser = argparse.ArgumentParser(description='Convert HFRL dataset to Qwen2.5 VL compatible LLaMA Factory format')
    ...
    parser.add_argument('--dataset_name', type=str, default="hfrl_data_qwen",
                        help='Dataset name')
...
def create_dataset_info(output_dir, dataset_name):
    dataset_info = {
        "alpaca_en": {
...
def convert_to_qwen_format(input_dir, output_dir, dataset_name):
    ...
    instruction_template = generate_instruction_template()
...
MEDIA_DIR_ABS="/home/ubuntu/HFRL"
OUTPUT_DIR_ABS="$LLAMA_FACTORY_ROOT/saves/qwen2_vl/lora/sft"
...
echo "Starting 8-card pure training mode (evaluation completely disabled)..."
echo "Model: /home/ubuntu/qwen2-vl"
echo "Dataset: {dataset_name}"
echo "Dataset directory (absolute): $DATASET_DIR_ABS"
echo "Media directory (absolute): $MEDIA_DIR_ABS"
echo "Output directory (absolute): $OUTPUT_DIR_ABS"
echo "---"
...
"""
    ...
    script_path = os.path.join(output_dir, "train_qwen.sh")
    with open(script_path, 'w') as f:
...
def main():
    ...
    print(f"===== Geographic Location Dataset Conversion Tool - Qwen2.5 VL Version =====")
    ...
    create_dataset_info(args.output_dir, args.dataset_name)

    success = convert_to_qwen_format(
        args.input_dir,
        args.output_dir,
        args.dataset_name
    )
    ...
    if success:
        ...
        print("\n=== Complete! ===")
        print("All files have been converted and are ready for Qwen2.5 VL model.")
        print(f"1. Run 'source {args.output_dir}/set_env.sh' to set environment variables")
        print(f"2. Run 'bash {args.output_dir}/train_qwen.sh' to start training")
        print("Note: Training script is optimized to reduce OOM risk using 8-card DeepSpeed ZeRO-3 training")
    else:
